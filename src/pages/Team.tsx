      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <header className="mb-24 md:mb-40">
          ...
          >
            ...
            <p className="text-slate-400 text-lg md:text-2xl max-w-2xl font-medium leading-relaxed uppercase tracking-tight">
              A decentralized collective of visionaries, engineers, and designers building the next generation of digital protocols.
            </p>
          </motion.div>
        ...
        <section className="mb-32 md:mb-56 grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 py-20 border-y border-white/5">
          {[
            ...
            >
              ...
              <div className="text-4xl md:text-6xl font-brutal tracking-tighter uppercase mb-2">{stat.value}</div>
        ...
        <section className="mb-32 md:mb-56">
          <div className="flex flex-col md:flex-row justify-between items-end mb-20 md:mb-32 gap-8">
            <div className="max-w-2xl">
              <span className="text-tertiary font-mono text-sm mb-6 block tracking-widest uppercase">// Core_Values</span>
              <h2 className="text-5xl md:text-8xl font-brutal tracking-tighter uppercase leading-none">Our <br /> <span className="text-slate-500">Culture</span></h2>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "Open Source", desc: "Everything we build is public by default. We believe in collective ownership of the future.", icon: Code2 },
              { title: "Collaboration", desc: "No silos. We work across departments to ensure every voice shapes the final protocol.", icon: Users2 },
              { title: "Innovation", desc: "We don't follow trends; we set them. Constant experimentation is in our DNA.", icon: Lightbulb },
            ].map((value, i) => (
              <motion.div 
                key={i}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <value.icon className="text-tertiary mb-8 group-hover:scale-110 transition-transform" size={32} />
                <h3 className="text-2xl font-brutal uppercase tracking-tighter mb-4">{value.title}</h3>
                <p className="text-slate-500 text-sm leading-relaxed uppercase font-medium">{value.desc}</p>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-32 md:mb-56">
          <div className="text-center mb-20 md:mb-32">
            <span className="text-tertiary font-mono text-sm mb-6 block tracking-widest uppercase">// Strategic_Guidance</span>
            <h2 className="text-5xl md:text-9xl font-brutal tracking-tighter uppercase leading-none">Mentors & <br /> <span className="text-slate-500">Advisors</span></h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6 md:gap-10">
            {[
              ...
              >
                ...
                <h4 className="text-sm md:text-base font-brutal uppercase tracking-tighter mb-2 relative z-10 group-hover:text-tertiary transition-colors">{mentor.name}</h4>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 relative z-10 mb-1">{mentor.role}</p>
                <p className="text-[8px] font-mono uppercase tracking-widest text-tertiary/60 relative z-10">{mentor.company}</p>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-32 md:mb-56 py-32 border-y border-white/5">
          <div className="text-center mb-20 md:mb-32">
            <span className="text-tertiary font-mono text-sm mb-6 block tracking-widest uppercase">// Onboarding_Pipeline</span>
            <h2 className="text-5xl md:text-8xl font-brutal tracking-tighter uppercase leading-none">Join the <br /> <span className="text-slate-500">Process</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 relative">
            ...
            {[
              { step: "01", title: "Apply", desc: "Submit your portfolio and vision for the protocol.", icon: Search },
              { step: "02", title: "Interview", desc: "Deep dive into your technical and cultural fit.", icon: MessageSquare },
              { step: "03", title: "Contribute", desc: "Complete a paid trial task on a live project.", icon: Zap },
              { step: "04", title: "Join Core", desc: "Welcome to the council. Start architecting.", icon: Rocket },
            ].map((item, i) => (
              <motion.div 
                key={i}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                ...
                <h3 className="text-2xl font-brutal uppercase tracking-tighter mb-4">{item.title}</h3>
                <p className="text-slate-500 text-xs uppercase tracking-widest leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-32 md:mb-56 py-32 md:py-48 border-b border-white/5 bg-white/[0.01] relative overflow-hidden">
          ...
          <div className="text-center mb-20 md:mb-32 relative z-10">
            <span className="text-tertiary font-mono text-sm mb-6 block tracking-widest uppercase">// Protocol_Builders</span>
            <h2 className="text-5xl md:text-9xl font-brutal tracking-tighter uppercase leading-none">Top <br /> <span className="text-slate-500">Contributors</span></h2>
          </div>
            ...
            {[
              ...
              >
                <div className="w-8 h-8 rounded-full bg-white/10 overflow-hidden border border-white/10 group-hover:border-tertiary/50">
                  <img src={`https://picsum.photos/seed/contrib${i}/100/100`} alt={contributor.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                </div>
                <div className="flex flex-col">
                  <span className="text-[10px] font-black uppercase tracking-widest">{contributor.name}</span>
                  <span className="text-[8px] font-mono text-slate-500 uppercase tracking-widest">{contributor.role} • {contributor.commits} commits</span>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        {filteredTeam.length === 0 && (
          <div className="text-center py-40 border border-dashed border-white/10 bg-white/[0.01] mb-32">
            ...
            <h3 className="text-3xl font-brutal uppercase tracking-tighter mb-2">No Architects Found</h3>
            <p className="text-slate-500 text-sm uppercase tracking-widest">The council is currently empty in this sector.</p>
            <button 
              ...
              <div className="flex flex-col items-center gap-8">
                <Magnetic strength={0.4}>
                  <button className="bg-black text-white px-20 py-10 font-black uppercase tracking-[0.2em] text-xl hover:scale-105 active:scale-95 transition-all shadow-2xl hover:shadow-black/50 flex items-center justify-center gap-3 mx-auto group/btn">
                    Apply for Council <ArrowUpRight size={24} className="group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1 transition-transform" />
                  </button>
                </Magnetic>
                <div className="flex items-center gap-4 text-black/60 text-[10px] font-black uppercase tracking-widest">
                  <span className="flex items-center gap-1"><CheckCircle2 size={12} /> Open for Developers</span>
                  <span className="flex items-center gap-1"><CheckCircle2 size={12} /> Designers</span>
                  <span className="flex items-center gap-1"><CheckCircle2 size={12} /> Builders</span>
                </div>
              </div>
            </motion.div>
          </div>

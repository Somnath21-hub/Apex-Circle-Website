          ...
          {[
            {
              icon: Target,
              title: 'Mission',
              desc: 'To empower every developer to build the future they envision.',
              color: 'text-primary',
            },
            {
              icon: Eye,
              title: 'Vision',
              desc: 'A world where innovation is accessible to everyone, everywhere.',
            ...
            <div
              key={i}
              className="bg-white/[0.02] border border-white/10 p-8 md:p-12 hover:border-white/30 transition-all group"
            >
              <div
                className={`w-14 h-14 md:w-16 md:h-16 bg-white/5 flex items-center justify-center mb-6 md:mb-8 ${item.color}`}
              >
                <item.icon size={28} md:size={32} />
              </div>
              <h3 className="text-2xl md:text-3xl font-brutal tracking-tight mb-4 uppercase">
                {item.title}
              </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24 items-center mb-24 md:mb-48">
            ...
            <div className="space-y-8 md:space-y-12">
              {[
                ...
                <div key={i} className="flex gap-6 md:gap-8 group">
                  <span className="text-primary font-mono text-lg md:text-xl pt-1">
                    [{step.year}]
                  </span>
                  <div>
          ...
          <div className="relative aspect-square bg-white/[0.02] border border-white/10 p-8 md:p-12 flex items-center justify-center overflow-hidden">
            ...
            <div className="text-center relative z-10">
              <div className="text-6xl sm:text-8xl md:text-[10rem] font-brutal leading-none text-white/10 mb-4">
                APEX
              </div>
              <p className="text-slate-500 font-mono text-xs md:text-sm uppercase tracking-[0.5em]">
                System_Online
              </p>
            </div>
          </div>
            ...
            {partners.map((partner, i) => (
              ...
              >
                <img
                  src={partner.logo}
                  alt={partner.name}
                  className="h-8 md:h-10  object-contain  group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500"
                />
              </div>
            ))}
          </div>
